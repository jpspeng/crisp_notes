---
title: 'CRISP Day 2 2026: Basic R Syntax'
output:
  html_document:
    toc: yes
    toc_float: yes
...
# Review: expressions and variables 
...
(TRUE/FALSE) variables, or even **dataframes** (data).
...
```{r}
# want select the 1st and 4th element 
# [] = subsetting 
my_vec[c(1,4)]
```
...
```{r}
my_vec[c(1,2,3,4)]
# we can make this a little shorter 
my_vec[1:4] # colon means to take all integers between the two numbers 
```
...
the vector containing the numbers 1 through 5. Multiply the vector by 10, 
...
# uses the mean() function on my_numbers 
mean(my_numbers)
```
...
**Example**: Write a loop to find the product of the first 8 odd numbers. 
...
```{r}
# nrow() and ncol() are functions that take in the dataframe as an argument
# outputs the rows and columns, respectively 
nrow(my_data)
ncol(my_data)
```
...
```{r}
# for binary variables, use the table() function 
table(my_data$tx)
```
...
4. (Optional) Write a loop to take add the first 10 even numbers, and then write 
another loop to divide the result repeatedly by the first 5 odd numbers. Then 
write code to round the result to three digits. What is the answer? 
...
5. (Optional) Write a function `count_rows_cols()` that takes as input a dataframe 
and outputs a vector which contains two items: the number of rows and the number 
of columns. 
...
5. (Optional) Write a function `find_mean_sd_age()` that takes as input a 
dataframe and outputs a vector which contains two items: the mean and the standard
